...
def symetry_Score(mask):

    a, b, c, d = extract_bboxes(np.array(mask))
    mask = mask[a:b, c:d]
    ...
    if imgP.shape[1] % 2 == 1:
        imgP = np.pad(imgP, [0, 1], "constant")
    mid = imgP.shape[1] // 2

    fold = imgP[:, :mid] | np.fliplr(imgP[:, mid:])
    if imgP.shape[0] % 2 == 1:
        imgP = np.pad(imgP, [1, 0], "constant")
    mid = fold.shape[0] // 2
    fold2 = fold[:mid, :] ^ np.flipud(fold[mid:, :])

    fold2 = np.where(fold2 != 0, 1, 0)
    return np.sum(fold2)
...
def get_color_variability(
    filtered_image, measure="variance"
):  # image needs to be filteredd for lesion
    ...
    if measure == "variance":
        ...
        rgb = (np.std(r[r > 0]), np.std(g[g > 0]), np.std(b[b > 0]))
    else:
        return
    return np.mean(rgb)
...
def boundry_check2(image, mask):
    ...
    average_boundry_rgb = sRGBColor(
        average_boundry[0], average_boundry[1], average_boundry[2]
    )
    ...
    return (
        delta_e_cie2000(color_dark_lab, color_boundry_lab),
        delta_e_cie2000(color_boundry_lab, color_skin_lab),
    )
...
def sym_detect(img):
    ...
    if first_size > second_size:
        ...
        second_half = np.pad(
            second_half, ((0, 0), (0, padd_rows)), "constant", constant_values=(0)
        )
    elif first_size < second_size:
        padd_rows = abs(first_size - second_size)
        first_half = np.pad(
            first_half, ((0, 0), (padd_rows, 0)), "constant", constant_values=0
        )
